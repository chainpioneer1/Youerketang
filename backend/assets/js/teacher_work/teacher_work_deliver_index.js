...
var selectedClassID = [];
var selectedClassIDArray = [0, 0];
var selectedClassIDCount = 0;
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.status == 'success') {
                        ...
                        $('.question_next_btn').css("display", "block");
                        isDecided = false;
...
function problem_set_output_content(problemSetList) {
    ...
    for (var i = 0; i < problemSetList.length; i++) {
        ...
        switch (parseInt(item.prob_type)) {
            case 1: // multiselect problem
                content_html += '<div class="question_list_item" itemid="' + (i + 1) + '">' +
                    '            <div class="option_btn" onclick="selectProblem(this)"></div>' +
                    '            <div class="prob_type_img" style="background: url(' + baseURL + '/assets/images/teacher_work/deliver/question_type.png);">选择</div>' +
                    ...
                    '        </div>';
                break;
            case 2: // yesno problem
                content_html += '<div class="question_list_item" itemid="' + (i + 1) + '">' +
                    '            <div class="option_btn" onclick="selectProblem(this)"></div>' +
                    '            <div class="prob_type_img" style="background: url(' + baseURL + '/assets/images/teacher_work/deliver/question_type.png);">判断</div>' +
                    '            <div class="prob_img" style="background: url(' + baseURL + item.prob_img + ');"></div>' +
                    '            <div class="speaker_btn" onclick="playSound(this)"></div>' +
                    '            <div class="seperate_container"></div>' +
                    '            <div class="answer_container">' +
            ...
            case 3: // audio recording
                content_html += '<div class="question_list_item" itemid="' + (i + 1) + '">' +
                    '            <div class="option_btn" onclick="selectProblem(this)"></div>' +
                    '            <div class="prob_type_img" style="background: url(' + baseURL + '/assets/images/teacher_work/deliver/question_type.png);">读一读</div>' +
                    '            <div class="prob_img" style="background: url(' + baseURL + item.prob_img + ');"></div>' +
                    '            <div class="speaker_btn" onclick="playSound(this)"></div>' +
                    '            <div class="seperate_container"></div>' +
                    '            <div class="answer_container">' +
                    ...
                    '        </div>';
                break;
            case 4: // audio recording text answer
                content_html += '<div class="question_list_item" itemid="' + (i + 1) + '">' +
                    '            <div class="option_btn" onclick="selectProblem(this)"></div>' +
                    '            <div class="prob_type_img" style="background: url(' + baseURL + '/assets/images/teacher_work/deliver/question_type.png);">读一读</div>' +
                    '            <div class="prob_img" style="background: url(' + baseURL + item.prob_img + ');"></div>' +
...
function checkClass(self) {
    var id = $(self).parent().attr("itemid");
    selectedClassIDArray[parseInt(id) - 1] = 1 - selectedClassIDArray[parseInt(id) - 1];
    if (selectedClassIDArray[id - 1] === 1) $('.class_list_item[itemid=' + id + ']').find('.check_container').css({background: 'url(' + baseURL + 'assets/images/teacher_work/deliver/check.png) no-repeat'});
    else if (selectedClassIDArray[id - 1] === 0) $('.class_list_item[itemid=' + id + ']').find('.check_container').css({background: 'url(' + baseURL + 'assets/images/teacher_work/deliver/uncheck.png) no-repeat'});
    selectedClassIDCount = 0;
    for (var i = 0; i < class_list.length; i++) {
        if (selectedClassIDArray[i] === 1) selectedClassIDCount++;
    }
    if (selectedClassIDCount > 0) isClassSelect = true;
}
...
function addPackage(self) {
    ...
    $('.all_option_btn').css({background: 'url(' + baseURL + 'assets/images/teacher_work/deliver/quanxuanru.png) no-repeat'});
...
function selectProblem(self) {
    ...
    if (selectedProblemCount === problemSetList.length) {
...
function layoutJob(self) {
    if ($('#timetext').val() == '') {
        ...
        var class_name = '';
        for (var i = 0; i < selectedClassIDArray.length; i++) {
            if (selectedClassIDArray[i] === 1) class_name += class_list[i].class_name + ",";
        }
        class_name = class_name.substring(0, class_name.length - 1);
        $('.confirm_class_name').html(class_name);
    }
}

function confirm(self) {
    var time = $('.datetime_text').val();
    if (time == '') {
    ...
    var classIdArray = [];
    for (var i = 0; i < selectedClassIDArray.length; i++) {
        if (selectedClassIDArray[i] == 1) {
            teacherIdArray.push(class_list[i].teacher_id);
            classIdArray.push(class_list[i].id);
        }
    }
...
function hideModal(self) {
    location.href = baseURL + "teacher_work/index/" + site_id;
    /*$('#success_modal').modal('hide');
    $('.confirm_container').css("display","none");
    $('.deliver_container[itemid=1]').css("display","block");
    $('.bg').css({background:'url(' + baseURL + 'assets/images/teacher_work/testing/bg1.png) no-repeat'});

    isClassSelect = false;
    selectedClassIDArray = [0,0];
    selectedClassIDCount = 0;
