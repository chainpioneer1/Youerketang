...
<div class="page-content-wrapper">
    ...
    <div class="page-content" style="min-height: 1305px;">
        <h1 class="page-title"><?php echo $this->lang->line('panel_title_43'); ?>
            <small></small>
        </h1>
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover" id="main_tbl">
                            ...
                            <tr>
                                ...
                                <th><?php echo $this->lang->line('education_course'); ?></th>
...
<style>
    ...
    .form-group div .img_preview[item_type="1"] {
            ...
            <div class="form-group">
                <div class="col-md-3 right-aligned">
                    <label for="cwImageUpload"
                           class="control-label"><?php echo $this->lang->line('upload_educourse_corner'); ?>:
                    </label>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-5" style="text-align: center;">
                    <div class="img_preview" item_type="1" style="background-size: 100% 100%!important;"></div>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    function update_item(self) {
        ...
        var item_path = self.getAttribute("item_path");
        ...
        $('div .img_preview[item_type=1]').css({background: 'url(' + item_path + ')'});

        if (item_path == '') item_path = '<?=$this->lang->line('NoFileSelected')?>';
        $('div[item_name=nameview1]').html(getFilenameFromURL(item_path));
    ...
    function update_perform(self) {
        ...
        $.ajax({
            url: baseURL + "admin/educourse/update_category",
            type: "POST",
            data: fdata,
            contentType: false,
            cache: false,
    ...
    function publish_item(self) {
		...
		if(!self.parentElement.parentElement.children[3].innerHTML){
			if (item_status == '0'){
			alert("<?= $this->lang->line('publish_package'); ?>");
			return;
			}
		}
        var msg_body = $('#item_publish_modal').find('.modal-body h4');
        if (item_status == '0')
            msg_body.html('<?= $this->lang->line('publish_educourse_confirm'); ?>');
        else
            msg_body.html('<?= $this->lang->line('unpublish_educourse_confirm'); ?>');
    ...
    function publish_perform(self) {
        ...
        $.ajax({
            type: "post",
            url: baseURL + "admin/educourse/publish_category",
            dataType: "json",
