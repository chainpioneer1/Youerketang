...
<div class="page-content-wrapper">
    ...
    <div class="page-content" style="min-height: 1305px;">
        <h1 class="page-title"><?php echo $this->lang->line('panel_title_33'); ?>
            <small></small>
        </h1>
        <div class="row">
            <div class="col-md-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <form action="#" class="form-horizontal">
                                    <div class="form-body">
                                        <div class="form-group">
                                            <label class="col-md-5 control-label"><?php echo $this->lang->line('school'); ?>
                                                :</label>
                                            <div class="col-md-7">
                                                <select class="form-control" id="school_search">
                                                    ...
                                                    foreach ($schools as $unit):
                                                        $j++;
                                                        echo '<option value="' . $unit->school_name . '">' . $unit->school_name . '</option>';
                                                    endforeach
                                                    ?>
                                                </select>
                                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="btn-group right-floated">
                                    <button class=" btn blue" onclick="add_item()">
                                        <i class="fa fa-plus"></i>&nbsp&nbsp<?php echo $this->lang->line('add_lesson'); ?>
                                    </button>
                                </div>
                            </div>
                            <!-------Table tool parts----------------->
...
<script>
    ...
    function add_item() {
        ...
        msg_title.html('<?= $this->lang->line('add_lesson'); ?>');
    ...
    function update_item(self) {
        ...
        var msg_title = $('#item_update_modal').find('.modal-header h4');
        msg_title.html('<?= $this->lang->line('update_lesson_information'); ?>');
    ...
    function update_perform(self) {
        ...
        $.ajax({
            url: baseURL + "admin/contents/update_lesson",
            type: "POST",
            data: fdata,
            contentType: false,
            cache: false,
    ...
    function publish_item(self) {
        ...
        if (item_status == '0')
            msg_body.html('<?= $this->lang->line('publish_lesson_confirm'); ?>');
        else
            msg_body.html('<?= $this->lang->line('unpublish_lesson_confirm'); ?>');
    ...
    function publish_perform(self) {
        ...
        $.ajax({
            type: "post",
            url: baseURL + "admin/contents/publish_lesson",
            dataType: "json",
    ...
    function update_order(self, step) {
        ...
        $.ajax({
            type: "post",
            url: baseURL + "admin/contents/update_lesson_order",
            dataType: "json",
            data: {school_id: school_id, data: write_info},
            success: function (res) {
                if (res.status == 'success') {
                    var table = document.getElementById("main_tbl");
                    var tbody = table.getElementsByTagName("tbody")[0];
                    tbody.innerHTML = res.data;
                    executionPageNation();
                    console.log('operation has been successed!')
                }
                else//failed
                {
                    alert("Cannot change lesson status.");
                }
            }
        });
    }
    ...
    $("#keyword_search").keyup(function () {///search for keyword
        var input, filter, school_txt;
        filter = this.value.toUpperCase();
        school_txt = $('#school_search').val().toUpperCase();
        search_action(filter, school_txt);
    });
    ...
    $("#school_search").on('change', function () {///search for area
        ...
        school_txt = $(this).val().toUpperCase();
        search_action(filter, school_txt);
    });
