<link rel="stylesheet" type="text/css" href="<?= base_url('assets/css/student/work-wrong.css') ?>">
<div class="bg"></div>
...
<script>
    ...
    var currentTaskId;
    var curId = 0;
    var bg_str = ['wrong-bg.png', 'bg-success.png', 'bg-fail.png'];
    ...
    $('.task-info[item_hover="1"]').on('click', function (object) {
        ...
        currentProblems[0] = JSON.parse($(this).attr('item_content'));
        var id = $(this).attr('item_col');
        curId = 0;
        switch(id){
            case '1':
                startWork(curId, true);
                break;
            case '5':
                startWork(curId);
                break;
...
//                    alert('请选择一个');
        ...
        $.ajax({
            type: "post",
            url: baseURL + "student/work/updateWrongProblem",
            dataType: "json",
            data: {
...
//                            showResult();
    ...
    function startWork(id, isReview) {
        reg_step = 2;
        if(isReview == undefined) isReview = false;

        if(isReview){
            $('.btn-prob-submit').css({'pointer-events':'none'});
            $('.problem-item .ans-img').css({'pointer-events':'none'});
        }else{
            $('.btn-prob-submit').css({'pointer-events':'auto'});
            $('.problem-item .ans-img').css({'pointer-events':'auto'});
        }
...
//            effecSoundPlay($(this).attr('item_src'), stopRecord);
    ...
    $('.ans-replay').on('click', function () {
        ...
        if (status != '1') {
            $(this).attr('item_sel', '1');
            var audio_path = baseURL + 'uploads/problem_set/answer/' + currentTaskId + "_" + curId + "_" + 'answer_mp3.mp3';
