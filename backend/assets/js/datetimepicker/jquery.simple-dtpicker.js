...
(function ($) {
    ...
    PickerHandler.prototype._relocate = function () {
        ...
        if ($input != null && $picker.data('isInline') === false) { // Float mode
            ...
            // input_outer_height = parseInt(input_outer_height) * 1.5;
            picker_outer_height = parseInt(picker_outer_height) * 1.5;
            ...
            if (true || parseInt($(window).height()) <= ($input.offset().top - $(document).scrollTop() + input_outer_height + picker_outer_height)) {
                // Display to top of an input-field
                $picker.parent().css('top', ($input.offset().top - (input_outer_height / 2) - picker_outer_height / 2 ) + 'px');
            } else {
            ...
            var scale = parseFloat($(window).width() / 800).toFixed(2);
            $($picker).parent().css({
                // left: '50%',
                // top: '50%',
                // transform: 'translate(-50%,-100%)',
                transform: 'scale(' + scale + ') translate(' + scale * 3 + '%,-23%)'
            });
        }
    };
