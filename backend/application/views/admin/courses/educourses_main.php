...
<div class="page-content-wrapper">
    ...
    <div class="page-content" style="min-height: 1305px;">
        <h1 class="page-title">
            <?php echo ($site_id == '1') ? $this->lang->line('panel_title_34') : $this->lang->line('panel_title_42'); ?>
            <small></small>
        </h1>
        <div class="row">
            <div class="col-md-12">
                                        ...
                                        <div class="form-group">
                                            <div class="col-md-12">
                                                <select class="form-control" id="school_search"
                                                        onchange="setOptions();">
                                                    <option value=""><?php echo $this->lang->line('search_school'); ?></option>
                                                    <?php
                                                    $j = 0;
                                                    foreach ($schools as $unit):
                                                        $j++;
                                                        echo '<option value="' . $unit->school_name . '" '
                                                            . ' item_id="' . $unit->id . '">'
                                                            . $unit->school_name . '</option>';
                                                    endforeach
                                                    ?>
                                                </select>
                                            </div>
                            ...
                            <div class="col-md-5">
                                <div class="btn-group right-floated">
                                    <button class=" btn blue" onclick="add_item()">
                                        <i class="fa fa-plus"></i>&nbsp&nbsp<?php echo $this->lang->line('add_normal_course'); ?>
                                    </button>
                                </div>
                            </div>
                            <!-------Table tool parts----------------->
                    ...
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover" id="main_tbl">
                            ...
                            <tr>
                                ...
                                <th><?php echo $this->lang->line('course_icon'); ?></th>
                                <th><?php echo $this->lang->line('course_content'); ?></th>
                                <th><?php echo $this->lang->line('course_status'); ?></th>
...
<div id="item_update_modal" class="modal fade" tabindex="-1" data-width="600">
    <div class="modal-header">
        ...
        <h3 class="modal-title"><?php echo $this->lang->line('message'); ?></h3>
    </div>
    <div class="modal-body">
        <form class="form-horizontal" enctype="multipart/form-data" action="" id="item_edit_submit_form" role="form"
            ...
            <h4 class="modal-sub-title"
                style="text-align: left;"><?php echo $this->lang->line('main_information'); ?></h4>
            <div class="form-group">
                <label class="col-md-4 control-label"><?php echo $this->lang->line('course_name'); ?>:</label>
                <div class="col-md-8">
            ...
            <div class="form-group">
                ...
                <div class="col-md-8">
                    <select class="form-control input-inline " type="text"
                            name="school_id" value="" onchange="setOptions();">
                        <?php
                        $j = 0;
                        foreach ($schools as $unit):
            ...
            <h4 class="modal-sub-title"
                style="text-align: left;"><?php echo $this->lang->line('content_upload'); ?></h4>
            <div class="form-group">
                <div class="col-md-3 right-aligned">
                    <label for="cwImageUpload"
                           class="control-label"><?php echo $this->lang->line('course_icon'); ?>:
                    </label>
                </div>
                <div class="col-md-4">
            ...
            <div class="form-group">
                <div class="col-md-3 right-aligned">
                    <label for="cwImageUpload"
                           class="control-label"><?php echo $this->lang->line('course_content'); ?>:
                    </label>
                </div>
                <div class="col-md-4">
                    <div class="fileinput fileinput-new" data-provides="fileinput"
...
<script>
    ...
    function add_item() {
        ...
        msg_title.html('<?= $this->lang->line('add_normal_course'); ?>');
    ...
    function update_item(self) {
        ...
        var msg_title = $('#item_update_modal').find('.modal-header h3');
        msg_title.html('<?= $this->lang->line('educourses_main_information'); ?>');
    ...
    function update_perform(self) {
        var item_id = self.getAttribute("item_id");
        var package_name = $('div[item_name=nameview5]').html();
        var icon_type = getFiletypeFromURL($('div[item_name=nameview4]').html());
        if (package_name == '<?=$this->lang->line('NoFileSelected')?>') {
            package_name = item_id;
        } else package_name = removeExtFromFilename(package_name);
        if (icon_type == '<?=$this->lang->line('NoFileSelected')?>') {
            icon_type = '';
        }
        ...
        fdata.append("icon_name", (icon_type != '') ? (package_name + '.' + icon_type) : '');
        fdata.append("package_name", (package_name != item_id) ? package_name : '');
        $.ajax({
            url: baseURL + "admin/educourse/update_course",
            type: "POST",
            data: fdata,
            contentType: false,
            cache: false,
    ...
    function publish_item(self) {
		...
		if(!self.parentElement.parentElement.children[3].children[0].children[0].getAttribute('src') 
			|| !self.parentElement.parentElement.children[4].innerHTML){
			if (item_status == '0'){
    ...
    function publish_perform(self) {
        ...
        $.ajax({
            type: "post",
            url: baseURL + "admin/educourse/publish_course",
            dataType: "json",
    ...
    function delete_perform(self) {
        ...
        jQuery.ajax({
            type: "post",
            url: baseURL + "admin/educourse/delete_course",
            dataType: "json",
    ...
    function update_order(self, step) {
        ...
        $.ajax({
            type: "post",
            url: baseURL + "admin/educourse/update_course_order",
            dataType: "json",
