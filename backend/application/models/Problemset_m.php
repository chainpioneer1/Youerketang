...
class Problemset_m extends MY_Model
{
    protected $_table_name = 'tbl_problem_set';
    ...
    public function getItems($site_id = 1)
    {
        ...
        $this->db->select('tbl_sites.title as site_name, tbl_package.title as name');
    ...
    function getProblemSetFromPackage($problem_ids){
        ...
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $whereCond = '( package_id = 0 ';
        foreach($problem_ids as $item){

            $whereCond .= ' or package_id = '.$item;
        }
        $whereCond .= ' ) ';
        $this->db->where($whereCond);
        $this->db->where('prob_status',1);
